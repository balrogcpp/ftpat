...
namespace rapidcsv
{
    ...
    void ToVal(const std::string& pStr, T& pVal) const
    {
      // try
      {
        ...
        if (std::is_same<T, float>::value)
        {
          ...
          fast_float::from_chars(pStr.data(), pStr.data() + pStr.size(), pVal, fast_float::chars_format::fixed);
          return;
        }
  ...
  public:
     ...
     */
    void Save(const std::string& pPath = std::string()) const
    {
      if (!pPath.empty())
      {
        mPath = pPath;
    ...
    std::vector<std::string> GetRowRaw(const size_t pRowIdx) const
    {
      const ssize_t rowIdx = pRowIdx + (mLabelParams.mColumnNameIdx + 1);
      // std::vector<T> row;
      // Converter<T> converter(mConverterParams);
    ...
    std::vector<std::string> GetRowRaw(const std::string& pRowName) const
    {
      ssize_t rowIdx = GetRowIdx(pRowName);
      if (rowIdx < 0)
      {
    ...
    std::string GetCellRaw(const size_t pColumnIdx, const size_t pRowIdx) const
    {
      const ssize_t columnIdx = pColumnIdx + (mLabelParams.mRowNameIdx + 1);
      const ssize_t rowIdx = pRowIdx + (mLabelParams.mColumnNameIdx + 1);
    ...
    std::string GetCellRaw(const std::string& pColumnName, const std::string& pRowName) const
    {
      const ssize_t columnIdx = GetColumnIdx(pColumnName);
      if (columnIdx < 0)
      {
      ...
      return GetCellRaw(columnIdx, rowIdx);
    }
    ...
    std::string GetCellRaw(const std::string& pColumnName, const size_t pRowIdx) const
    {
      const ssize_t columnIdx = GetColumnIdx(pColumnName);
      if (columnIdx < 0)
      {
      ...
      return GetCellRaw(columnIdx, pRowIdx);
    }
     ...
     * @returns column name.
     */
    std::string GetColumnName(const ssize_t pColumnIdx) const
    {
      const ssize_t columnIdx = pColumnIdx + (mLabelParams.mRowNameIdx + 1);
      if (mLabelParams.mColumnNameIdx < 0)
      {
     ...
     * @returns row name.
     */
    std::string GetRowName(const ssize_t pRowIdx) const
    {
      const ssize_t rowIdx = pRowIdx + (mLabelParams.mColumnNameIdx + 1);
      if (mLabelParams.mRowNameIdx < 0)
      {
  ...
  private:
    ...
    mutable LabelParams mLabelParams;
    mutable SeparatorParams mSeparatorParams;
    mutable ConverterParams mConverterParams;
    std::vector<std::vector<std::string>> mData;
#ifndef M_XTD
    std::unordered_map<std::string, size_t> mColumnNames;
    std::unordered_map<std::string, size_t> mRowNames;
#else
